class Complement
  ...
  def initialize(complement_string)
    @complement_string = complement_string
  end
  
  def noun
    @complement_string.split.select { |word| $dictionary.noun_words.include?(word) }[0]
  end
  
  def correct
    nominal = @complement_string
...
class Subject
  ...
  def noun
    @subject_string.split.select { |word| $dictionary.noun_words.include?(word) }[0]
  end  
  
  def correct
    nominal = @subject_string
    return nil if nominal == ""
    return nominal if $dictionary.verbe.keys.include?(nominal.split.first)
