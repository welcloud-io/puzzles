class Verb
  ...
  def correct
    first_word = @context.subject.split.first
...
class Sentence
  ...
  def subject
    @sentence.split(self.verb.to_s)[SUJET].strip
  end
  ...
  def complement
    (@sentence.split(self.verb.to_s)[COMPLEMENT] || "").strip    
  end
  
  def correct
    [correct_nominal(self.subject), self.verb.correct, correct_nominal(self.complement)].join(' ').strip
    #~ [correct_nominal(self.subject), correct_verb(self.subject), correct_nominal(self.complement)].join(' ').strip
  end
...
def corrected_sentence(sentence_string, verbe, adjectifs, noms)
  ...
  #~ corrected_sentence = [correct_nominal(sentence.subject), correct_verb(sentence.subject), correct_nominal(sentence.complement)].join(' ').strip
