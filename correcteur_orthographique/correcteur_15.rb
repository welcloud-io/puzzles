...
def correct_nominal(nominal)
    ...
    adjectifs = get_adjectifs(nominal_words)
    
    noun, genre, nombre = get_nominal_noun_genre_et_nombre(nominal_words)    

    nominal_words.map do |word|
      case
        when word == noun then correct_noun(word, nombre)
        when adjectifs.include?(word)  then correct_adjectif(word, genre, nombre)
        else word
      end
    end.join(' ')
