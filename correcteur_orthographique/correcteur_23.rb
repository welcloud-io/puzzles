...
class Subject < NominalGroup
  ...
  def person
    first_word_string = @words.first.to_s
    person = first_word_string if $dictionary.pronoun?(first_word_string)
    person = $dictionary.articles[first_word_string].to_sym if $dictionary.article?(first_word_string)
    return person
  end
