...
def get_verb_from(sentence)
  @verbe_conjugue.values.uniq.select { |declinaison| sentence.split.include?(declinaison) }[0]
end
...
def pronoun?(sujet)
  @verbe_conjugue.keys.include?(sujet.to_sym)
end
...
def find_noun(nominal)
  nominal.reject { |word| (@verbe_conjugue_au_present + @adjectifs_conjugues + @pronouns.keys +  @all_but_nouns += @nombre.keys ).include?(word) }[0]
  #~ nominal.select { |word| (@noms_conjugues.flatten).include?(word) }[0]
end
