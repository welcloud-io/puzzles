...
def get_subject_from(sentence)
  ...
  sentence.split(get_verb_from(sentence.split).join)[0].strip.split
end

def get_verb_from(sentence)
  sentence = sentence.join(' ')
  @conjugaisons.uniq.select { |declinaison| sentence.split.include?(declinaison) }
end

def get_complement_from(sentence)
  sentence = sentence.join(' ')  
  verbe = get_verb_from(sentence.split).join
...
def correct_verb_with_pronoun(pronoun)
  @conjugaisons[@pronouns[pronoun]]
end

def correct_verb_with_article(article)
  il = 2;  ils = 5
  if @nombre[article] == 'singulier'
      return @conjugaisons[il]
  else
      return @conjugaisons[ils]
  end  
end
