...
class Noun < Word
  ...
  def gender
    @dictionary.nouns[@word_string][:genre]
  end
...
class Article < Word
  ...
  def person
    return 'il' if dictionary.articles[@word_string.first.to_s] == :singular
    return 'ils' if dictionary.articles[@word_string.first.to_s] == :plural    
  end
end
...
class Subject
  ...
  def corrected(dictionary)
    @words.map! do |word|
      word.corrected(gender(@words, dictionary), number(@words, dictionary))  
    end    
  end
end
...
class Complement
  ...
  def corrected(dictionary, subject, verb)
    @words.map! do |word|
      case 
        when dictionary.verb_of_state?(verb) then ref = subject
        else ref = self
        end
      word.corrected(ref.gender(ref.words, dictionary), ref.number(ref.words, dictionary))
    end    
  end
end
...
def corrected_sentence(sentence_string, verb, adjectives, nouns)
  ...
  words = sentence_string.split.map do |word_string| 
    case 
    when dictionary.verb?(word_string) then Verb.new(word_string, dictionary)
    when dictionary.adjective?(word_string) then Adjective.new(word_string, dictionary)
    when dictionary.noun?(word_string) then Noun.new(word_string, dictionary)
    else Word.new(word_string,dictionary)
    end
  end
  ...
  subject = Subject.new(words[0..verb_index-1])
  verb = words[verb_index..verb_index]
  complement = Complement.new(words[verb_index+1..-1])
  
  verb.map! do |verb| 
    verb.corrected(subject.person(dictionary), nil)
  end
  
  (subject.corrected(dictionary) + verb + complement.corrected(dictionary, subject, verb[0].to_s)).join(" ").strip
