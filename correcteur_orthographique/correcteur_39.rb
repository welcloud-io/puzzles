...
def gender(words, dictionary)
  ...
  words.each { |word| return dictionary.pronouns[word][:gender] if dictionary.pronoun?(word) }  
end

def number(words, dictionary)
  words.each { |word| return dictionary.articles[word] if dictionary.article?(word) } 
  words.each { |word| return dictionary.pronouns[word][:number] if dictionary.pronoun?(word) }   
end
...
def corrected_sentence(sentence_string, verb, adjectives, nouns)
  ...
  complement.map! do |word|
    case
      when not(dictionary.verb_of_state?(verb[0])) && dictionary.adjective?(word) then dictionary.adjectives[word][gender(complement, dictionary)][number(complement, dictionary)]
      when (dictionary.verb_of_state?(verb[0])) && dictionary.adjective?(word) then dictionary.adjectives[word][gender(subject, dictionary)][number(subject, dictionary)]
      when not(dictionary.verb_of_state?(verb[0])) && dictionary.noun?(word) then dictionary.nouns[word][number(complement, dictionary)]
      else word
    end
  end  
