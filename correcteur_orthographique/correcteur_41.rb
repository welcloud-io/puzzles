...
class Word
  ...
  def corrected(person, dictionary)
    dictionary.verb[person]
  end
...
def corrected_sentence(sentence_string, verb, adjectives, nouns)
  ...
  words = sentence_string.split.map { |word_string| Word.new(word_string) }
  ...
  subject.map! do |word|
    case
      when dictionary.adjective?(word.to_s) then dictionary.adjectives[word.to_s][gender(subject, dictionary)][number(subject, dictionary)]
      when dictionary.noun?(word.to_s) then dictionary.nouns[word.to_s][number(subject, dictionary)]
      else word
    end
  end
  
  verb.map! do |word| 
    word.corrected(person(subject, dictionary), dictionary)
  end
  
  complement.map! do |word|
    case
      when not(dictionary.verb_of_state?(verb[0])) && dictionary.adjective?(word.to_s) then dictionary.adjectives[word.to_s][gender(complement, dictionary)][number(complement, dictionary)]
      when (dictionary.verb_of_state?(verb[0].to_s)) && dictionary.adjective?(word.to_s) then dictionary.adjectives[word.to_s][gender(subject, dictionary)][number(subject, dictionary)]
      when not(dictionary.verb_of_state?(verb[0])) && dictionary.noun?(word.to_s) then dictionary.nouns[word.to_s][number(complement, dictionary)]
      else word
    end
  end  
