...
def corrected_sentence(sentence_string, verb, adjectives, nouns)
  ...
  verb.map! do |word| 
    word.corrected(subject.person(dictionary), nil, dictionary)
  end
  
  
  complement.map! do |word|
    case 
      when dictionary.verb_of_state?(verb[0].to_s) then word.corrected(subject.gender(subject.words, dictionary), subject.number(subject.words, dictionary), dictionary)
      else word.corrected(gender(complement, dictionary), number(complement, dictionary), dictionary)
    end
  end  
  
  (subject.corrected(dictionary) + verb + complement).join(" ").strip
