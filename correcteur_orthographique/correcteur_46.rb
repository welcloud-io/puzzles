...
class Subject
  ...
  def corrected(dictionary)
    @words.map! do |word|
      word.corrected(gender(@words, dictionary), number(@words, dictionary), dictionary)  
    end    
  end
end
...
class Complement
  ...
  @words.map! do |word|
    case 
      when dictionary.verb_of_state?(verb) then word.corrected(subject.gender(subject.words, dictionary), subject.number(subject.words, dictionary), dictionary)
      else word.corrected(gender(@words, dictionary), number(@words, dictionary), dictionary)
    end
  end    
  end
end
...
def corrected_sentence(sentence_string, verb, adjectives, nouns)
  ...
  verb.map! do |verb| 
    verb.corrected(subject.person(dictionary), nil, dictionary)
  end
