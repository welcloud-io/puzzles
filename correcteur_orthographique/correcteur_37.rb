...
def corrected_sentence(sentence_string, verb, adjectives, nouns)
  ...
  words.map! do |word|
    case
      ...
      when dictionary.adjective?(word) then dictionary.adjectives[word][gender(words, dictionary)][number(words, dictionary)]
      when dictionary.noun?(word) then dictionary.nouns[word][number(words, dictionary)]
      else word
    end
  end
